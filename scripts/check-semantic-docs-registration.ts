...
if (!existsSync(manifestPath)) {
  ...
  try {
    ...
    if (
      ...
      !manifest.every((entry) => typeof entry === "string")
    ) {
      failures.push(
        "scripts/semantic-doc-checks.json must be an array of checker filenames",
      );
    } else {
    ...
    failures.push(
      `scripts/semantic-doc-checks.json is invalid JSON: ${String(error)}`,
    );
  }
}
...
if (JSON.stringify(registeredCheckers) !== JSON.stringify(sortedUnique)) {
  failures.push(
    "semantic checker registration must be sorted and contain no duplicates",
  );
}

const omitted = discoveredCheckers.filter(
  (checker) => !registeredCheckers.includes(checker),
);
const stale = registeredCheckers.filter(
  (checker) => !discoveredCheckers.includes(checker),
);
if (omitted.length > 0) {
...
if (stale.length > 0) {
  failures.push(
    `registered semantic checker(s) missing from scripts/: ${stale.join(", ")}`,
  );
}
...
const infrastructureScripts = {
  ...
  "validate:pipeline-contract": "node scripts/check-docs-pipeline-contract.ts",
};
for (const [name, expected] of Object.entries(infrastructureScripts)) {
...
if (scripts["validate:semantic-docs"] !== expectedAggregate) {
  failures.push(
    `package.json validate:semantic-docs must be ${JSON.stringify(expectedAggregate)}`,
  );
}
...
for (const checker of discoveredCheckers) {
  ...
  if (scripts[`validate:${name}`] !== expected) {
    failures.push(
      `package.json must preserve focused command validate:${name}`,
    );
  }
}
...
if (validate !== expectedValidate) {
  failures.push(
    `package.json validate must be ${JSON.stringify(expectedValidate)}`,
  );
}
for (const checker of discoveredCheckers) {
  const name = checker.slice("check-".length, -".ts".length);
  if (validate.includes(`validate:${name}`)) {
    failures.push(
      `package.json validate must not run focused semantic command validate:${name}`,
    );
  }
}
if (
  scripts["validate:build"] !==
  "pnpm sync:content && astro check && astro build"
) {
  failures.push(
    "package.json validate:build must remain safe from a clean checkout",
  );
}
if (scripts["validate:build:prepared"] !== "astro check && astro build") {
  failures.push(
    "package.json validate:build:prepared must build pre-generated content once",
  );
}
if (
  scripts["validate:a11y"] !==
  "pnpm validate:build && node scripts/check-a11y-smoke.ts"
) {
  failures.push(
    "package.json validate:a11y must remain safe from a clean checkout",
  );
}
if (scripts["validate:a11y:prepared"] !== "node scripts/check-a11y-smoke.ts") {
  failures.push(
    "package.json validate:a11y:prepared must check the existing aggregate build",
  );
}
...
if (!/pull_request:\n    branches: \[main\]\n  push:/.test(workflow)) {
  failures.push(
    "docs-validate workflow must run for every pull request into main",
  );
}
for (const requiredPath of [
  ...
  if (count !== 1) {
    failures.push(
      `docs-validate workflow must include ${requiredPath} in push path filters`,
    );
  }
}
