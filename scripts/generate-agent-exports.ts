...
- For repository \`example\` and branch \`feature/auth\`, mappings are: bare \`default\` + \`preserve\` | \`example/feature/auth\`; bare \`default\` + \`flatten\` | \`example/feature-auth\`; bare \`branch\` + \`preserve\` | \`feature/auth\`; bare \`branch\` + \`flatten\` | \`feature-auth\`; bare \`repo-branch\` + \`preserve\` | \`example-feature/auth\`; bare \`repo-branch\` + \`flatten\` | \`example-feature-auth\`; Non-bare \`default\` + \`preserve\` | \`feature/auth\`.
