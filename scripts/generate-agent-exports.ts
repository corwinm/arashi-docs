...
- \`--no-hooks\` is create-only; \`--no-hook-input\` is shared by create and remove. Remove dry-run returns source-aware previews. Configured-create dry-run performs no hook discovery, has an empty hook ledger, and no hook preview. Public metadata uses \`sourceKind: "inline-config"\`, \`sourceOwnerKind\`, and \`sourceOwnerName\`; singular \`sourceScriptPath\` is null, while ambiguity can expose ordered native \`sourceScriptPaths\`. Outcomes, previews, diagnostics, and logs never reveal snippet command text.
