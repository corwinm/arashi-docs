...
- Optional \`aw add\` onboarding requires human stdin and stdout TTY eligibility. The top-level setup prompt defaults to no; declining preserves the minimal \`path\` and \`gitUrl\` add. \`--json\`, \`--force\`, and non-TTY runs preserve minimal add without discovery or onboarding.
- Setup is repository-owned and limited to \`copy\`, \`symlink\`, and four repository lifecycle hooks: \`pre-create\`, \`post-create\`, \`pre-remove\`, and \`post-remove\`. Suggestions remain unselected; a bounded root-only scan provides suggestions, and Arashi never reads or displays candidate contents. Manual entries receive canonical path validation, and manual dependency-directory choices produce a warning. Setup-script context never prefills or infers a hook command.
- For each lifecycle, choose exactly one user-supplied inline command or editable active native script. Create scripts use the active configuration root at \`.arashi/hooks/<lifecycle>.<repo><ext>\`; remove scripts use the runtime-resolved target repository at \`.arashi/hooks/<lifecycle><ext>\`, and linked remove uses the active child. POSIX creates one executable \`.sh\` with mode \`0755\`; Windows creates one \`.ps1\` that is runtime-ready. Each scaffold is a safe silent no-op with no rename/chmod activation, and Arashi never overwrites existing files. A sanitized summary never includes bodies or generated contents.
...
- Onboarding has no prompt-time writes. One final sanitized confirmation authorizes one config save plus transaction-owned scripts; cancellation enters rollback. Existing-entry editing remains follow-up issue #316.
