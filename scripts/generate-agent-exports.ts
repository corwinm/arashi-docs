...
- Create base branches with \`aw create <target> --base <branch>\` or workspace-generic \`defaults.create.baseBranch\`. Precedence is CLI > configuration > legacy behavior; selected repositories resolve local then origin before hooks or mutation, while reused targets keep their ancestry. See [Create command Markdown](${site}/commands/create.md).
