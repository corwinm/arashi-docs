...
- \`aw\` is a shorter alias for \`arashi\`. See [Getting started](${site}/getting-started/) for installation and collision guidance.
...
- Use \`arashi completion bash|zsh|fish\` for native completion. Static completion works outside workspaces; dynamic repositories, groups, worktrees, branches, paths, shells, and constrained values are local and read-only with a 200 ms whole-query budget. Use \`command arashi\` in activation code to bypass wrappers.
...
- In configured workspaces only, \`repos.<name>.copy\` is a direct array and \`repos.<name>.symlink\` is a direct array. Entries use the same relative path from the Git-primary child checkout into each new worktree. Choose \`copy\` for an independent, isolated file and \`symlink\` only to share intentional state or dependencies. Globs are not supported, path remapping is not supported, and standalone mode is not supported. Use lifecycle hooks for globs, remapping, external sources, interpolation, required entries, generated files, or conditional behavior. \`arashi doctor\` non-mutatively diagnoses materialization source availability and destination safety without repair or mutation.
- Configured child construction runs \`pre-create\`, \`copy\`, \`symlink\`, then \`post-create\`; \`--no-hooks\` does not disable materialization. Missing sources are skipped, Arashi never overwrites an existing destination, and every destination must remain inside the worktree. A native \`symlink\` fails when platform policy or the filesystem rejects it, with no copy, hard-link, or junction fallback. Materialization does not fall back to the caller's checkout or another source repository. \`arashi create --dry-run\` previews the ordered materialization plan in declaration order before any worktree or file mutation.
- Create base branches with \`arashi create <target> --base <branch>\` or workspace-generic \`defaults.create.baseBranch\`. Precedence is CLI > configuration > legacy behavior; selected repositories resolve local then origin before hooks or mutation, while reused targets keep their ancestry. See [Create command Markdown](${site}/commands/create.md).
- Standalone create base selection is CLI-only and invocation-only: it does not read or persist \`defaults.create.baseBranch\` and creates no \`.arashi\` state.
- Dry-run reports the selected repositories' resolved bases and planned actions without mutation. JSON includes the optional structured \`base\` object only when a base is requested; success entries are at \`data.base.repositories\`, and resolution failures are at \`error.details.repositories\` with affected repositories only in selected-set order.
- For older Arashi releases, pre-create the target branch from the desired base in managed children and the parent separately, then use \`--conflict REUSE_EXISTING\`. Use this only for absent targets or independently verified ancestry; \`REUSE_EXISTING\` does not repair or validate ancestry.
