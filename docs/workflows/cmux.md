...
For explicit `--tab`, the supported managed equivalent is a cmux workspace / vertical tab in the active cmux session. Arashi does not launch the containing standalone Ghostty window, and a cmux failure never falls through to one. See the [Launching reference](/reference/launching/) for the full matrix and failure boundaries.
