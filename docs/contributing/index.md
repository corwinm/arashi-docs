---
title: Contributing
description: How contributors add docs safely while preserving information architecture.
draft: false
sidebar:
  hidden: false
---

This section defines the contributor workflow and maintenance expectations for the Arashi documentation site.

## Contributing Workflow

- [How to Add Pages](/contributing/how-to-add-pages/)
- [Navigation Rules](/contributing/navigation-rules/)
- [Page Template](/contributing/page-template/)
- [Content Style](/contributing/content-style/)
- [Review Checklist](/contributing/review-checklist/)
- [Validation Troubleshooting](/contributing/validation-troubleshooting/)
- [Ownership](/contributing/ownership/)
- [Agents](/workflows/agents-and-specs/)

## Related

- [Getting Started](/getting-started/)
- [Workflows](/workflows/)
- [Commands](/commands/)
