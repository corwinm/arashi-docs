...
Enable `arashi switch` to change the current shell directory through an installed wrapper, and manage startup activation for both the wrapper and native completion. Supported integration enables both `arashi` and `aw`, with `arashi` remaining canonical.
...
`arashi shell install` owns both activation lines in one managed block: the parent-shell wrapper and completion sourcing. That block enables parent-shell behavior and native completion for both executable names. Repeated installation replaces the block idempotently instead of adding duplicate lines. Restart your shell or source the updated startup file afterward.
