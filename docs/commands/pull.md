...
- `--jobs` applies only after parent handling, configuration reload, and managed-ignore reconciliation. Arashi runs children serially when it cannot establish independent Git identities; results retain configured order even if concurrent pulls finish out of order. With independent concurrent children, the first failure stops new starts; already-started pulls finish before the failure summary. Serial mode continues through the selected children.
